...
class GameProvider extends ChangeNotifier {
  ...
  void _initStars() {
    ...
    for (int i = 0; i < 80; i++) {
      stars.add(StarParticle(
          x: _rng.nextDouble(), y: _rng.nextDouble(), speed: 0.0004,
          size: 0.5 + _rng.nextDouble() * 0.8, opacity: 0.2 + _rng.nextDouble() * 0.3, layer: 0));
    }
    for (int i = 0; i < 40; i++) {
      stars.add(StarParticle(
          x: _rng.nextDouble(), y: _rng.nextDouble(), speed: 0.001,
          size: 0.8 + _rng.nextDouble() * 1.2, opacity: 0.3 + _rng.nextDouble() * 0.4, layer: 1));
    }
    for (int i = 0; i < 15; i++) {
      stars.add(StarParticle(
          x: _rng.nextDouble(), y: _rng.nextDouble(), speed: 0.003,
          size: 1.2 + _rng.nextDouble() * 1.5, opacity: 0.6 + _rng.nextDouble() * 0.4, layer: 2));
    }
  }
  ...
  void _spawnBullets() {
    ...
    switch (state.currentWeapon) {
      ...
      case WeaponType.rapidFire:
        bullets.add(Bullet(x: player.x, y: player.y - 0.03, vy: -0.028, color: Colors.yellowAccent));
        break;
      ...
      case WeaponType.laser:
        ...
        bullets.add(Bullet(x: player.x, y: player.y - 0.5, vy: -0.05, color: Colors.redAccent));
        break;
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      particles.add({
        ...
        'vx': cos(angle) * 0.006,
        'vy': sin(angle) * 0.006,
        'life': 0.4, 'color': Colors.white, 'size': 2.5, 'decay': 0.06,
      });
    }
  }
  ...
  void _damageObstacle(Obstacle obs) {
    ...
    if (obs.hp <= 0) {
      shakeIntensity = (obs.wallTier == WallTier.armored) ? 14.0 :
                       (obs.wallTier == WallTier.reinforced) ? 10.0 : 5.0;
  ...
  void _spawnHitSparks(double x, double y, Color color) {
    for (int i = 0; i < 8; i++) {
      ...
      particles.add({
        ...
        'life': 0.7, 'color': Color.lerp(color, Colors.white, 0.6)!,
        'size': 2.5 + _rng.nextDouble() * 2, 'decay': 0.05,
      });
    }
  }
  ...
  void _spawnExplosionParticles(double x, double y, {bool intense = false}) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final colors = [
        const Color(0xFFFF2D55), const Color(0xFFFF6B2B),
        const Color(0xFFFFB020), Colors.white,
        const Color(0xFFFFFF00),
      ];
      particles.add({
        'x': x, 'y': y,
        'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.003,
        'life': 1.0, 'color': colors[_rng.nextInt(colors.length)],
        'size': intense ? (5.0 + _rng.nextDouble() * 10) : (4.0 + _rng.nextDouble() * 7),
    ...
    for (int i = 0; i < (intense ? 20 : 8); i++) {
      ...
      particles.add({
        ...
        'life': 1.0, 'color': const Color(0xFFFF8800),
        'size': 2.0 + _rng.nextDouble() * 4, 'decay': 0.018,
      });
    }
  }
  ...
  void _spawnDebrisParticles(double x, double y, Color color, WallTier tier) {
    final count = tier == WallTier.armored ? 24 :
                  tier == WallTier.reinforced ? 16 : 8;
    for (int i = 0; i < count; i++) {
      final angle = _rng.nextDouble() * 2 * pi;
      final speed = 0.004 + _rng.nextDouble() * 0.018;
      final size = tier == WallTier.armored ? (6.0 + _rng.nextDouble() * 10) :
                   tier == WallTier.reinforced ? (4.0 + _rng.nextDouble() * 7) :
    ...
    for (int i = 0; i < count ~/ 2; i++) {
      ...
      particles.add({
        'x': x, 'y': y,
        'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.005,
        'life': 0.8, 'color': Colors.white,
        'size': 1.5, 'decay': 0.04,
      });
    }
  }
  ...
  void _spawnBombExplosionParticles(double x, double y) {
    ...
    for (int i = 0; i < 80; i++) {
      ...
      final colors = [Colors.white, const Color(0xFFFF6B2B), const Color(0xFFFFD60A),
                      const Color(0xFFFF00FF), const Color(0xFF00FFFF)];
      particles.add({
        'x': x, 'y': y,
        'vx': cos(angle) * speed, 'vy': sin(angle) * speed,
        'life': 1.0, 'color': colors[_rng.nextInt(colors.length)],
        'size': 4.0 + _rng.nextDouble() * 12, 'decay': 0.012,
      });
    }
    // Rising fire plume
    for (int i = 0; i < 40; i++) {
      particles.add({
        ...
        'life': 1.0, 'color': const Color(0xFFFF4400),
        'size': 5.0 + _rng.nextDouble() * 8, 'decay': 0.01,
      });
    }
  }
  ...
  void _spawnChest(double x, double y) {
    ...
    chests.add(TreasureChest(
      ...
      speed: 0.002, reward: reward, coinAmount: coinAmt,
    ));
  }
    ...
    switch (player.trailStyle) {
      case TrailStyle.clean:
        ...
        if (_rng.nextDouble() < 0.5)
          trail.add(TrailPoint(x: player.x, y: player.y + 0.022, life: 0.6, size: 2.5, color: Colors.white));
        break;
      case TrailStyle.scatter:
        for (int i = 0; i < 3; i++) {
          final vx = (_rng.nextDouble() - 0.5) * 0.012;
          trail.add(TrailPoint(x: player.x, y: player.y + 0.02, life: 0.8, size: 2.5 + _rng.nextDouble() * 2, color: color, vx: vx));
        }
        break;
  ...
  void _spawnGapWall() {
    ...
    for (final obs in obstacles) {
      ...
      if (obs.x < 0.02) {
        for (final obs2 in obstacles) {
          if (obs2.type == ObstacleType.laserWall && obs2.x > 0.05 &&
    ...
    if (gapLeft > 0.02) {
      obstacles.add(Obstacle(
        x: 0, y: spawnY, width: gapLeft, height: h,
        speed: spd, type: ObstacleType.laserWall,
        color: tierData.color, wallTier: tier,
      ));
    }
    final rightStart = gapLeft + gapWidth;
    if (rightStart < 0.98) {
      obstacles.add(Obstacle(
        x: rightStart, y: spawnY, width: 1.0 - rightStart, height: h,
        speed: spd, type: ObstacleType.laserWall,
        color: tierData.color, wallTier: tier,
      ));
    }
  ...
  void _spawnZigzag() {
    ...
    if (gap1Left > 0.02)
      obstacles.add(Obstacle(x: 0, y: -0.06, width: gap1Left, height: h, speed: spd, type: ObstacleType.laserWall, color: tierData.color, wallTier: tier));
    final gap1Right = gap1Left + gw;
    if (gap1Right < 0.98)
      obstacles.add(Obstacle(x: gap1Right, y: -0.06, width: 1.0 - gap1Right, height: h, speed: spd, type: ObstacleType.laserWall, color: tierData.color, wallTier: tier));

    final gap2Center = leftSide ? 0.70 : 0.20;
    final gap2Left = gap2Center - gw / 2;
    if (gap2Left > 0.02)
      obstacles.add(Obstacle(x: 0, y: -0.42, width: gap2Left, height: h, speed: spd, type: ObstacleType.laserWall, color: tierData.color, wallTier: tier));
    final gap2Right = gap2Left + gw;
    if (gap2Right < 0.98)
      obstacles.add(Obstacle(x: gap2Right, y: -0.42, width: 1.0 - gap2Right, height: h, speed: spd, type: ObstacleType.laserWall, color: tierData.color, wallTier: tier));
  }
  ...
  void _spawnSweepBeam() {
    ...
    obstacles.add(Obstacle(
      x: 0, y: yPos.clamp(0.15, 0.75), width: 1.0, height: 0.032,
  ...
  void _spawnCoin() {
    if (_rng.nextDouble() < 0.3) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        coins.add(Coin(
            x: (baseX + offset).clamp(0.1, 0.9), y: -0.05 - i * 0.001,
            speed: 0.003 + state.difficulty * 0.0008, pulsePhase: i * 0.4));
      }
    } else {
      coins.add(Coin(x: 0.1 + _rng.nextDouble() * 0.8, y: -0.05,
  ...
  void _spawnPowerUp() {
    if (_rng.nextDouble() < 0.55) {
      powerUps.add(PowerUp(
          x: 0.1 + _rng.nextDouble() * 0.8, y: -0.05, speed: 0.003,
  ...
  void _spawnCoinParticles(double x, double y) {
    for (int i = 0; i < 10; i++) {
      ...
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.012,
                     'life': 1.0, 'color': const Color(0xFFFFD60A), 'size': 3.0 + _rng.nextDouble() * 3, 'decay': 0.04});
    }
  }

  void _spawnPowerUpParticles(double x, double y, PowerUpType type) {
    final color = type == PowerUpType.shield ? const Color(0xFF4D7CFF)
        : type == PowerUpType.slowTime ? const Color(0xFF8B5CF6) : const Color(0xFFFF2D55);
    for (int i = 0; i < 14; i++) {
      final angle = _rng.nextDouble() * 2 * pi;
      final speed = 0.005 + _rng.nextDouble() * 0.01;
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.008,
                     'life': 1.0, 'color': color, 'size': 3.0 + _rng.nextDouble() * 4, 'decay': 0.035});
    }
  }
  ...
  void _spawnChestParticles(double x, double y) {
    for (int i = 0; i < 20; i++) {
      ...
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.012,
                     'life': 1.0, 'color': colors[_rng.nextInt(colors.length)],
                     'size': 4.0 + _rng.nextDouble() * 6, 'decay': 0.025});
    }
  }
