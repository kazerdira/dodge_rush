...
class GameProvider extends ChangeNotifier {
  ...
  static const double _bossRespawnInterval = 45.0; // respawn every 45s after kill
  ...
  bool isBreathWindow = false;
  double _breathCooldown = 0.0;
  int _waveCount = 0;
  static const int _waveBeforeBreathe = 4;

  double nearMissFlash = 0.0;
  ...
  // ── REBALANCED OBSTACLE INTERVALS ─────────────────────────────────────────
  // More breathing room early on; still intense in later sectors.
  double get _obstacleInterval {
    ...
    switch (state.lastPattern) {
      ...
      case PatternType.minefield:
        ...
        break;
      default:
        base = 1.4;   // was 1.0 — gives player time to breathe
    }
    base -= (state.sector - 1) * 0.07;  // slightly less aggressive than before
    return base.clamp(0.65, 2.8);        // floor raised from 0.55 → 0.65
  }
  ...
  void _initStars() {
    ...
    for (int i = 0; i < 80; i++) {
      stars.add(StarParticle(
          ...
          speed: 0.0004, size: 0.5 + _rng.nextDouble() * 0.8,
          opacity: 0.2 + _rng.nextDouble() * 0.3, layer: 0));
    }
    for (int i = 0; i < 40; i++) {
      stars.add(StarParticle(
          x: _rng.nextDouble(), y: _rng.nextDouble(),
          speed: 0.001, size: 0.8 + _rng.nextDouble() * 1.2,
          opacity: 0.3 + _rng.nextDouble() * 0.4, layer: 1));
    }
    for (int i = 0; i < 15; i++) {
      stars.add(StarParticle(
          x: _rng.nextDouble(), y: _rng.nextDouble(),
          speed: 0.003, size: 1.2 + _rng.nextDouble() * 1.5,
          opacity: 0.6 + _rng.nextDouble() * 0.4, layer: 2));
    }
  }
  ...
  void detonateBomb() {
    ...
    freezeFrameTimer = _freezeDuration;
    shakeIntensity = 10.0;
    tensionLevel = 0.0;
    ...
    for (int i = 0; i < 12; i++) {
      ...
      const cols = [Colors.white, Colors.white, Color(0xFFFF6B2B), Color(0xFFFFD60A)];
      particles.add({
        'x': player.x, 'y': player.y,
        'vx': cos(angle) * speed, 'vy': sin(angle) * speed,
        'life': 1.0, 'color': cols[_rng.nextInt(cols.length)],
        'size': 7.0 + _rng.nextDouble() * 14, 'decay': 0.014,
        'shape': 'ember', 'angle': angle, 'spin': 0.0,
      });
    }
  ...
  void _tick() {
    ...
    final firstSpawnReady = state.sector >= 3 && !bossSpawned && _gameTime > 55.0;
    final respawnReady = bossFullyGone && bossKillCount > 0
        && state.sector >= 3
    ...
    if (firstSpawnReady || respawnReady) {
      ...
      final rateScale = max(1.2, 3.5 - state.sector * 0.3 - bossKillCount * 0.2);
      boss = BossShip(
        hp: hpScale,
        maxHp: hpScale,
        fireRate: rateScale,
    ...
    if (boss != null && !boss!.isFullyDead) {
      ...
      if (!boss!.isDead) {
        ...
        if (boss!.fireTimer < 0.8)
          boss!.warningFlash = (boss!.warningFlash + _tickRate * 3).clamp(0.0, 1.0);
        if (boss!.fireTimer <= 0) {
          boss!.warningFlash = 0;
          bossFire();
        ...
        if (boss!.hp <= 0) {
          ...
          for (int i = 0; i < 16; i++) {
            ...
            const cols = [Colors.white, Color(0xFFFF2D55), Color(0xFFFF6B00), Color(0xFFFFD60A)];
            particles.add({
              'x': boss!.x, 'y': boss!.y,
              'vx': cos(a) * spd, 'vy': sin(a) * spd,
              'life': 1.0, 'color': cols[_rng.nextInt(cols.length)],
              'size': 8.0 + _rng.nextDouble() * 16, 'decay': 0.014,
              'shape': 'ember', 'angle': a, 'spin': 0.0,
            });
          }
          shockwaves.add(Shockwave(x: boss!.x, y: boss!.y, radius: 0.01, life: 1.0, color: Colors.white));
          shockwaves.add(Shockwave(x: boss!.x, y: boss!.y, radius: 0.02, life: 1.0, color: const Color(0xFFFF2D55)));
          state.score += 5000;
      ...
      if (!boss!.isDead && boss!.y > -0.05) {
        ...
        for (final b in bullets) {
          ...
          if (b.x > boss!.x - bossW / 2 &&
            ...
            spawnHitSparks(boss!.x + (_rng.nextDouble() - 0.5) * 0.12, boss!.y, const Color(0xFFFF2D55));
    ...
    if (gauntletActive && !escaped) {
      ...
      if (gauntletTimer >= _gauntletDuration) {
        ...
        for (int i = 0; i < 18; i++) {
          ...
          particles.add({
            'x': player.x, 'y': player.y,
            'vx': cos(a) * spd, 'vy': sin(a) * spd - 0.01,
            'life': 1.0, 'color': cols[_rng.nextInt(cols.length)],
            'size': 6.0 + _rng.nextDouble() * 12, 'decay': 0.012,
            'shape': 'ember', 'angle': a, 'spin': 0.0,
          });
        }
    ...
    if (nearMissCooldown <= 0) {
      for (final obs in obstacles) {
        ...
        if (obs.type == ObstacleType.laserWall) {
          if (player.y > obs.y - 0.06 && player.y < obs.y + obs.height + 0.06) {
        ...
        if (dist < 0.045 && dist > 0.01) {
          ...
          state.score += bonus;
          if (nearMissStreak >= 2) onRewardCollected?.call('🔥 NEAR MISS ×$nearMissStreak  +$bonus');
          break;
        }
      }
    }
    ...
    if (_timeSinceLastCoin >= _coinInterval) { spawnCoin(); _timeSinceLastCoin = 0; }
    if (_timeSinceLastPowerUp >= _powerUpInterval) { spawnPowerUp(); _timeSinceLastPowerUp = 0; }
    ...
    for (final coin in coins) { coin.y += coin.speed * slowedSpeed; coin.pulsePhase += _tickRate * 3; }
    coins.removeWhere((c) => c.y > 1.1 || c.collected);
    for (final pu in powerUps) { pu.y += pu.speed * slowedSpeed; pu.pulsePhase += _tickRate * 2; }
    powerUps.removeWhere((p) => p.y > 1.1 || p.collected);
    for (final c in chests) { c.y += c.speed * slowedSpeed; c.pulsePhase += _tickRate * 2.5; }
    ...
    if (_bombKillCursor < _bombKillQueue.length) {
      ...
      for (int i = _bombKillCursor; i < end; i++) {
        ...
        if (i % 2 == 0)
          shockwaves.add(Shockwave(x: cx, y: cy, radius: 0.01, life: 0.55, color: obs.color));
        _bombKillCursor++;
      }
    }
    ...
    if (shakeIntensity > 0) {
      ...
      shakeOffset = Offset(
          ...
          (_rng.nextDouble() - 0.5) * shakeIntensity);
      if (shakeIntensity < 0.4) { shakeIntensity = 0; shakeOffset = Offset.zero; }
    }
