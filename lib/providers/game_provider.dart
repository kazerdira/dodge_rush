...
class GameProvider extends ChangeNotifier {
  ...
  void startGame({SkinType skin = SkinType.phantom}) {
    ...
    particles.clear(); trail.clear(); ghostImages.clear();
    shakeIntensity = 0;
    _timeSinceLastObstacle = 0; _timeSinceLastCoin = 0; _timeSinceLastPowerUp = 0;
    _gameTime = 0; _animTick = 0; _ghostTimer = 0;
    _targetX = null;
    _lastGapCenter = 0.5;
    _initStars();
    _gameLoop?.cancel();
  ...
  void pauseGame() { state.isPaused = !state.isPaused; notifyListeners(); }

  void stopGame() {
    _gameLoop?.cancel(); _gameLoop = null;
    state.isPlaying = false; state.isGameOver = true;
    notifyListeners(); onGameOver?.call();
  }
  ...
  void _tick() {
    ...
    obstacles.removeWhere((o) => o.y > 1.2 || (o.type == ObstacleType.sweepBeam && o.sweepDone && o.y > 0.2));

    for (final coin in coins) { coin.y += coin.speed * state.speed * speedMult; coin.pulsePhase += _tickRate * 3; }
    coins.removeWhere((c) => c.y > 1.1 || c.collected);
    for (final pu in powerUps) { pu.y += pu.speed * state.speed * speedMult; pu.pulsePhase += _tickRate * 2; }
  ...
  void _spawnGapWall() {
    ...
    for (final obs in obstacles) {
      ...
      if (obs.y < 0 || obs.y > 0.75) continue;
      if (obs.y > bestY) {
        // Left wall has x ≈ 0; right wall has x > 0.1
        if (obs.x < 0.02) {
          final gapLeft = obs.x + obs.width;
          for (final obs2 in obstacles) {
            if (obs2.type == ObstacleType.laserWall && obs2.x > 0.1 && (obs2.y - obs.y).abs() < 0.02) {
              referenceCenter = (gapLeft + obs2.x) / 2.0;
              bestY = obs.y;
              break;
            }
          }
    ...
    // New gap center must be within reach of the reference row
    ...
    // Clamp so the full gap fits on screen
    gapCenter = gapCenter.clamp(gapWidth / 2 + 0.03, 1.0 - gapWidth / 2 - 0.03);
    _lastGapCenter = gapCenter;

    final gapLeft = (gapCenter - gapWidth / 2).clamp(0.05, 0.75);
    final spd = 0.0042 + state.difficulty * 0.0016;
    const color = Color(0xFFFF2D55);

    // Left wall: 0 to gapLeft
    if (gapLeft > 0.02) {
      obstacles.add(Obstacle(x: 0, y: -0.055, width: gapLeft, height: 0.042, speed: spd, type: ObstacleType.laserWall, color: color));
    }
    // Right wall: gapLeft+gapWidth to 1.0
    final rightStart = gapLeft + gapWidth;
    if (rightStart < 0.98) {
      obstacles.add(Obstacle(x: rightStart, y: -0.055, width: 1.0 - rightStart, height: 0.042, speed: spd, type: ObstacleType.laserWall, color: color));
    }
  }

  // Pattern 2: Zigzag — two walls but spaced FAR apart vertically so player has time to move
  // FIX: was spawning both walls nearly simultaneously. Now they are far enough apart
  // that the player fully passes the first before the second arrives.
  void _spawnZigzag() {
    final spd = 0.0038 + state.difficulty * 0.0014;
    const color = Color(0xFFFF8C00);
    final leftSide = _rng.nextBool();

    // Gap width generous — zigzag is about movement, not squeezing
    const gw = 0.28;

    // First wall
    final gap1Center = leftSide ? 0.18 : 0.72;
    final gap1Left = gap1Center - gw / 2;
    if (gap1Left > 0.02) obstacles.add(Obstacle(x: 0, y: -0.06, width: gap1Left, height: 0.04, speed: spd, type: ObstacleType.laserWall, color: color));
    final gap1Right = gap1Left + gw;
    if (gap1Right < 0.98) obstacles.add(Obstacle(x: gap1Right, y: -0.06, width: 1.0 - gap1Right, height: 0.04, speed: spd, type: ObstacleType.laserWall, color: color));

    // Second wall: spawn much further back so player has full travel time between them
    // At spd ~0.004, screen height is 1.0 — 0.35 gap means ~2 seconds between walls at normal speed
    final gap2Center = leftSide ? 0.72 : 0.18;
    final gap2Left = gap2Center - gw / 2;
    if (gap2Left > 0.02) obstacles.add(Obstacle(x: 0, y: -0.40, width: gap2Left, height: 0.04, speed: spd, type: ObstacleType.laserWall, color: color));
    final gap2Right = gap2Left + gw;
    if (gap2Right < 0.98) obstacles.add(Obstacle(x: gap2Right, y: -0.40, width: 1.0 - gap2Right, height: 0.04, speed: spd, type: ObstacleType.laserWall, color: color));
  }
  ...
  void _spawnPulseGate() {
    final centerX = 0.22 + _rng.nextDouble() * 0.56; // 0.22 to 0.78, always reachable
    final spd = 0.003 + state.difficulty * 0.0008;
        ...
        switch (obs.type) {
          ...
          case ObstacleType.sweepBeam:
              ...
              if (!obs.sweepDone) {
                ...
                final beamAtPlayerY = py + ph / 2 > obs.y && py - ph / 2 < obs.y + obs.height;
  ...
  void _spawnCoinParticles(double x, double y) {
    for (int i = 0; i < 10; i++) {
      ...
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.012, 'life': 1.0, 'color': const Color(0xFFFFD60A), 'size': 3.0 + _rng.nextDouble() * 3});
    }
  }
  ...
  void _spawnExplosionParticles(double x, double y) {
    for (int i = 0; i < 22; i++) {
      ...
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed, 'life': 1.0, 'color': colors[_rng.nextInt(colors.length)], 'size': 4.0 + _rng.nextDouble() * 7});
    }
  }
  ...
  void _spawnPowerUpParticles(double x, double y, PowerUpType type) {
    ...
    for (int i = 0; i < 14; i++) {
      ...
      particles.add({'x': x, 'y': y, 'vx': cos(angle) * speed, 'vy': sin(angle) * speed - 0.008, 'life': 1.0, 'color': color, 'size': 3.0 + _rng.nextDouble() * 4});
    }
  }

  @override
  void dispose() { _gameLoop?.cancel(); super.dispose(); }
}
