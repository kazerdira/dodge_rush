...
// Wide delta wings with forward-swept mandibles, deep bevelled armor plates,
// red glowing trim lines, central red reactor eye, mechanical claw arms,
// command tower.
...
void drawBossShip(
  ...
  // ── 1. DANGER AURA ────────────────────────────────────────────────────────
  final aura = (1.0 - hpRatio) * 0.35 + boss.warningFlash * 0.5;
  if (aura > 0.05) {
    paint.maskFilter = MaskFilter.blur(BlurStyle.normal, 70);
    paint.color = const Color(0xFFCC1100).withOpacity(aura * 0.35 * opacity);
    canvas.drawOval(
  ...
  for (int side = -1; side <= 1; side += 2) {
    ...
    paint.color = const Color(0xFF08080F).withOpacity(opacity);
    canvas.drawPath(wing, paint);
    paint.style = PaintingStyle.fill;
    ...
    paint.color =
        const Color(0xFFFF2200).withOpacity((0.70 + pulse * 0.30) * opacity);
    canvas.drawLine(Offset(s * w * 0.15, -h * 0.30),
        Offset(s * w * 1.20, -h * 0.72), paint);
    paint.maskFilter = null;
    paint.strokeWidth = 1.8;
    paint.color = const Color(0xFFFF5533).withOpacity(opacity);
    canvas.drawLine(Offset(s * w * 0.15, -h * 0.30),
        Offset(s * w * 1.20, -h * 0.72), paint);
    ...
    paint.color = const Color(0xFFFF2200).withOpacity(0.55 * opacity);
    canvas.drawLine(
        Offset(s * w * 1.05, h * 0.38), Offset(s * w * 0.72, h * 0.55), paint);
    paint.maskFilter = null;
    paint.strokeWidth = 1.2;
    paint.color = const Color(0xFFFF3311).withOpacity(0.80 * opacity);
    canvas.drawLine(
        Offset(s * w * 1.05, h * 0.38), Offset(s * w * 0.72, h * 0.55), paint);

    // ── PANEL SEAM LINES (structural rib feel)
    paint.strokeWidth = 0.8;
    paint.color = const Color(0xFF0D0D1A).withOpacity(0.9 * opacity);
    for (int p = 1; p <= 5; p++) {
      final t = p / 6.0;
      canvas.drawLine(
    ...
    for (int block = 0; block < 3; block++) {
      ...
      paint.color = const Color(0xFF525262).withOpacity(0.55 * opacity);
      canvas.drawRRect(
    ...
    for (int d = 0; d < 6; d++) {
      ...
      paint.color = const Color(0xFF08000A).withOpacity(opacity);
      canvas.drawCircle(Offset(vx, vy), 3.8, paint);
      paint.maskFilter = MaskFilter.blur(BlurStyle.normal, 6);
      paint.color =
          const Color(0xFFFF2200).withOpacity((0.55 + pulse * 0.45) * opacity);
      canvas.drawCircle(Offset(vx, vy), 3.2, paint);
      paint.maskFilter = null;
      paint.color = Colors.white.withOpacity(0.90 * opacity);
    ...
    paint.maskFilter = MaskFilter.blur(BlurStyle.normal, 6);
    paint.color =
        const Color(0xFFFF2200).withOpacity((0.55 + pulse * 0.35) * opacity);
    canvas.drawLine(
        Offset(s * w * 0.88, h * 0.78 + mSwing),
    ...
    for (int v = 0; v < 3; v++) {
      ...
      paint.maskFilter = MaskFilter.blur(BlurStyle.normal, 5);
      paint.color =
          const Color(0xFFFF2200).withOpacity((0.65 + pulse * 0.35) * opacity);
  ...
  for (final pr in plateRects) {
    ...
    paint.color = const Color(0xFF565668).withOpacity(0.65 * opacity);
    canvas.drawRRect(
        RRect.fromRectAndRadius(pr, const Radius.circular(2)), paint);
    // Plate top sheen
    paint.color = Colors.white.withOpacity(0.07 * opacity);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
  ...
  for (int rib = 0; rib < 7; rib++) {
    ...
    // Shadow cut
    paint.color = const Color(0xFF05050A).withOpacity(opacity);
  ...
  paint.color = const Color(0xFFFF3311).withOpacity(0.85 * opacity);
  for (int side = -1; side <= 1; side += 2) {
    canvas.drawLine(Offset(side * w * 0.37, -h * 0.58),
        Offset(side * w * 0.30, h * 0.68), paint);
  }
  ...
  paint.style = PaintingStyle.stroke;
  paint.strokeWidth = 1.5;
  paint.color = const Color(0xFF070710).withOpacity(opacity);
  canvas.drawPath(hull, paint);
  paint.style = PaintingStyle.fill;
  ...
  paint.color = const Color(0xFF606072).withOpacity(opacity);
  canvas.drawCircle(Offset(0, -h * 0.08), w * 0.155, paint);
  paint.strokeWidth = 1.5;
  paint.color = Colors.white.withOpacity(0.20 * opacity); // rim light
  canvas.drawArc(
      ...
      paint);
  paint.strokeWidth = 1.5;
  paint.color = const Color(0xFF2A2A38).withOpacity(opacity);
  ...
  paint.color =
      const Color(0xFFFF1A00).withOpacity((0.65 + pulse * 0.35) * opacity);
  ...
  // Glare spot
  paint.color = Colors.white.withOpacity(0.72 * opacity);
  ...
  if (boss.warningFlash > 0.05) {
    ...
    paint.color =
        const Color(0xFFFF0000).withOpacity(boss.warningFlash * opacity);
  ...
  if (boss.warningFlash > 0.05) {
    paint.maskFilter = MaskFilter.blur(BlurStyle.normal, 22);
    paint.color =
        const Color(0xFFFF0033).withOpacity(boss.warningFlash * opacity);
    canvas.drawCircle(Offset(0, h * 1.06), w * 0.12, paint);
    paint.maskFilter = null;
    paint.color = Colors.white.withOpacity(boss.warningFlash * 0.9 * opacity);
  ...
  paint.color = const Color(0xFFFF2200).withOpacity(0.90 * opacity);
  ...
  if (!boss.isDead) {
    ...
    paint.color = const Color(0xFF0D0D15).withOpacity(0.92 * opacity);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromCenter(
    ...
    drawText(canvas, '✦  IMPERIAL DREADNOUGHT  ✦', Offset(0, barActualY - 15),
        const Color(0xFFFF3311).withOpacity(opacity), 9,
        letterSpacing: 2.5);
  }
