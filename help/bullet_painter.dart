...
void _drawNeedle(Canvas canvas, double cx, double cy, Color color, Paint paint) {
  ...
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 5, height: 20), paint);
  paint.maskFilter = null;
  // Core shaft
  paint.color = color;
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 2.5, height: 14), paint);
  // Hot tip
  paint.color = Colors.white;
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy - 5), width: 2.5, height: 6), paint);
  // Tip bright point
...
void _drawPlasma(Canvas canvas, double cx, double cy, Color color, Paint paint) {
  ...
  ).createShader(Rect.fromCenter(center: Offset(cx, cy + 6), width: 8, height: 14));
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy + 5), width: 8, height: 12), paint);
  paint.shader = null;
  // Core orb
...
void _drawShell(Canvas canvas, double cx, double cy, Color color, Paint paint) {
  ...
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy + 8), width: 10, height: 16), paint);
  paint.maskFilter = null;
  // Shell body — dark metallic
  paint.shader = LinearGradient(
    colors: [const Color(0xFFCCCCDD), const Color(0xFF555566), const Color(0xFF222233)],
    begin: Alignment.topLeft,
    end: Alignment.bottomRight,
  ).createShader(Rect.fromCenter(center: Offset(cx, cy), width: 8, height: 16));
  canvas.drawRRect(
    RRect.fromRectAndRadius(Rect.fromCenter(center: Offset(cx, cy + 2), width: 7, height: 13), const Radius.circular(3)),
    paint,
  );
  paint.shader = null;
  ...
  ).createShader(Rect.fromCenter(center: Offset(cx, cy - 6), width: 7, height: 8));
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy - 5), width: 7, height: 10), paint);
  paint.shader = null;
  // Tip gleam
...
void _drawBeam(Canvas canvas, double cx, double cy, Color color, Paint paint) {
  ...
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 14, height: 28), paint);
  paint.maskFilter = null;

  // Transparent beam shaft
  paint.shader = LinearGradient(
    colors: [color.withOpacity(0.0), color.withOpacity(0.55), color.withOpacity(0.0)],
    begin: Alignment.bottomCenter,
    end: Alignment.topCenter,
  ).createShader(Rect.fromCenter(center: Offset(cx, cy), width: 6, height: 26));
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 6, height: 26), paint);
  paint.shader = null;

  // Inner bright core (semi-translucent)
  paint.color = Colors.white.withOpacity(0.50);
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 3, height: 18), paint);
...
void _drawCannon(Canvas canvas, double cx, double cy, Color color, Paint paint) {
  ...
  paint.maskFilter = const MaskFilter.blur(BlurStyle.normal, 10);
  paint.color = color.withOpacity(0.35);
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy + 6), width: 16, height: 14), paint);
  paint.maskFilter = null;

  // Metallic sphere body
  paint.shader = RadialGradient(
    colors: [const Color(0xFF888899), const Color(0xFF333344), const Color(0xFF0A0A14)],
  ...
  canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy + 9), width: 14, height: 4), paint);
  paint.style = PaintingStyle.fill;
