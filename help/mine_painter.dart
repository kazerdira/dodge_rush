...
void _drawProximityMine(Canvas canvas, double cx, double cy, double r,
  ...
  paint.shader = RadialGradient(
    colors: [const Color(0xFF2A2A30), const Color(0xFF111115), const Color(0xFF050508)],
  ...
  for (int i = 0; i < 6; i++) {
    final a = (pi / 3 * i) - pi / 6;
    canvas.drawCircle(Offset(cos(a) * r * 0.78, sin(a) * r * 0.78), r * 0.07, paint);
  }
...
void _drawTrackerMine(Canvas canvas, double cx, double cy, double r,
  ...
  paint.shader = RadialGradient(
    colors: [const Color(0xFF1A2030), const Color(0xFF0A0E1A), const Color(0xFF020508)],
    center: const Alignment(-0.2, -0.3),
  ).createShader(Rect.fromCircle(center: Offset.zero, radius: r));
  canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: r * 2, height: r * 2.1), paint);
  paint.shader = null;
  ...
  for (int arc = 0; arc < 2; arc++) {
    ...
    canvas.drawArc(Rect.fromCircle(center: Offset.zero, radius: r * 0.72), a, 0.6, false, paint);
  }
  paint.style = PaintingStyle.fill;
...
void _drawClusterMine(Canvas canvas, double cx, double cy, double r,
  ...
  paint.shader = RadialGradient(
    colors: [const Color(0xFF1E0A0A), const Color(0xFF100505), const Color(0xFF050101)],
  ).createShader(Rect.fromCircle(center: Offset.zero, radius: r));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int s = 0; s < 6; s++) {
      ...
      paint.style = PaintingStyle.stroke;
      paint.strokeWidth = 1.5;
      canvas.drawLine(Offset(chamberX + cos(sa) * chamberR * 0.7, chamberY + sin(sa) * chamberR * 0.7), Offset(tipX, tipY), paint);
      paint.style = PaintingStyle.fill;
    }
  ...
  for (int i = 0; i < 3; i++) {
    final a = (2 * pi / 3 * i) - pi / 2;
    canvas.drawLine(Offset.zero, Offset(cos(a) * r * 0.95, sin(a) * r * 0.95), paint);
  }
  paint.maskFilter = null;
  paint.style = PaintingStyle.fill;
  ...
  final tp = TextPainter(
    text: TextSpan(
      text: 'CLUSTER',
      style: TextStyle(color: color.withOpacity(0.7 * opacity), fontSize: 7, fontWeight: FontWeight.w900, letterSpacing: 1.5),
    ),
...
// ── DEATH EXPLOSION ────────────────────────────────────────────────────────
void _drawMineExplosion(Canvas canvas, double cx, double cy, double r, double t, Color color) {
  final paint = Paint()..style = PaintingStyle.fill;
