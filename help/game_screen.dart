                ...
                if (_game.state.isShieldActive)
                  Positioned.fill(child: IgnorePointer(child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(color: AppTheme.accentAlt.withOpacity(0.5), width: 2),
                    ),
                  ))),
                            ...
                            child: Center(
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(
                                      color: (isBomb ? const Color(0xFFFF6B00) : AppTheme.coinColor).withOpacity(0.7),
                                      width: 1.5),
                                  boxShadow: [BoxShadow(
                                      color: (isBomb ? const Color(0xFFFF6B00) : AppTheme.coinColor).withOpacity(0.3),
                                      blurRadius: 20)],
                                ),
                                child: Text(
                                  _rewardText!,
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: bombColor.withOpacity(_pressed ? 1.0 : hasBombs ? 0.5 + _pulse.value * 0.3 : 0.3),
                        width: _pressed ? 3.0 : 2.0),
                    boxShadow: hasBombs ? [
                      BoxShadow(
                        color: bombColor.withOpacity(_pressed ? 0.7 : 0.15 + _pulse.value * 0.2),
...
class _SectorBannerState extends State<_SectorBanner> with SingleTickerProviderStateMixin {
          ...
          child: IgnorePointer(child: Center(child: Opacity(
            ...
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Text('ENTERING SECTOR $_lastSector', style: const TextStyle(
                    color: AppTheme.accent, fontSize: 11, letterSpacing: 6, fontWeight: FontWeight.w900)),
                const SizedBox(height: 4),
                Text(_getSectorName(_lastSector), style: const TextStyle(
                    color: AppTheme.textPrimary, fontSize: 22, letterSpacing: 2, fontWeight: FontWeight.w900)),
              ]),
            ),
          ))),
        );
  ...
  String _getSectorName(int sector) {
    const names = ['', 'VOID EXPANSE', 'ASTEROID BELT', 'NEBULA CORE', 'DEBRIS FIELD', 'DEEP SPACE'];
  ...
  Widget build(BuildContext context) {
    return Positioned.fill(child: Container(
      ...
      child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('◼ PAUSED', style: TextStyle(color: AppTheme.accent, fontSize: 36, fontWeight: FontWeight.w900, letterSpacing: 6)),
        const SizedBox(height: 8),
        Text('SECTOR ${game.state.sector}', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12, letterSpacing: 4)),
  ...
  return GestureDetector(
    ...
    child: Container(
      ...
      child: Center(child: Text(label, style: TextStyle(
          color: fg, fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 2))),
    ),
  );
}
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Container(
            ...
            child: Text('SECTOR ${game.state.sector}', style: const TextStyle(
                color: AppTheme.accent, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.w900)),
          ),
          const Spacer(),
          Container(
            ...
            child: Text(_formatScore(game.state.score), style: const TextStyle(
                color: AppTheme.textPrimary, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: 2)),
          ),
          const Spacer(),
          ...
          if (game.state.bombs > 0)
            Container(
              ...
              child: Row(children: [
                ...
                Text('×${game.state.bombs}', style: const TextStyle(
                    color: Color(0xFFFF6B2B), fontWeight: FontWeight.w900, fontSize: 12)),
              ]),
            ),
          Container(
            ...
            child: Row(children: [
              ...
              Text('${game.state.coins}', style: const TextStyle(
                  color: AppTheme.coinColor, fontWeight: FontWeight.w800, fontSize: 13)),
            ]),
          ),
        ]),
        ...
        if (game.state.isShieldActive || game.state.isSlowActive) ...[
          ...
          Row(children: [
            if (game.state.isShieldActive) ...[
              _PowerBar(label: 'SHL', color: AppTheme.accentAlt, progress: game.state.shieldTimer / 6.0),
              const SizedBox(width: 8),
            ],
            if (game.state.isSlowActive)
              _PowerBar(label: 'SLW', color: AppTheme.slowColor, progress: game.state.slowTimer / 5.0),
          ]),
        ],
        ...
        if (game.state.combo >= 3) ...[
          ...
          Container(
            ...
            child: Text('✕${game.state.combo}  COMBO', style: const TextStyle(
                color: AppTheme.coinColor, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 2)),
          ),
        ],
      ]),
    );
